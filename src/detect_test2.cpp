/*
   Copyright (c) 2020, Riku Yokoo and Naoya Takahashi , Masato Kubotera , Yuuta Mibuchi , Kazuki Shirasu
All rights reserved.
*/
#include <ros/ros.h>
...
/*--rosからopencvに変換-----------------------------------*/
  ...
  for(int j=0;j<contours_b.size();j++){
    area=contourArea(contours_b.at(j));
    if(max_area<area){
        max_area=area;
        max_area_contour=j;
    }
  ...
  if(max_area_contour != -1){
    int count=contours_b.at(max_area_contour).size();
    double x=0;
    double y=0;
    for(int k=0;k<count;k++){
      x+=contours_b.at(max_area_contour).at(k).x;
      y+=contours_b.at(max_area_contour).at(k).y;
    }
    x/=count;
    y/=count;
